<script lang="ts">
	...
	let wrapperCls = $derived(cn(wrapperClass, classWrapper));
	let containerCls = $derived(cn(containerClass, classContainer));
	let codeInputCls = $derived(cn(codeInputClass, className));
	...
	let joined = $derived(values.join(''));
...
<!--@component
	- Concept Established by TreltaSev
    - Component Originally Created by Amaan Dhanani
	- TreltaSev remade component, generated by SvelteForge🔥
	- Amaan Dhanani has updated this since

    @file CodeInput.svelte
    @description
